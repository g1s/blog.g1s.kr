...
var G1sUtil = new function() {
	...
	this.isLAlphabet = function(s) {
		return /^[a-z]+$/.test(s);
	}
	this.isUAlphabet = function(s) {
		return /^[A-Z]+$/.test(s);
	}
	this.isAlphabet = function(s) {
		return /^[a-zA-Z]+$/.test(s);
	}
	this.indexOfUpper = function(s) {
		return /[A-Z]/.exec(s)
	}
	this.indexOfLower = function(s) {
		return /[a-z]/.exec(s)
	}
	this.indexOfAlphabet = function(s) {
		return /[a-zA-Z]/.exec(s)
	}
	this.split = function(s, c, d) {
		var r = s.split(c);
		var t = s;

		if (this.isEmpty(d) || d == 0) {
		} else if (d < 0) {
		...
		for (var i = 0; i < j$.length; i++) {
			...
			for ( var d in data) {
				if (d.startsWith(pfix) 
						&& G1sUtil.isUAlphabet(d.substr(3, 1))
						&& d != gdata2('id') 
						&& d != gdata2('class')
						&& d != gdata2('if') 
